...
let { price, couponV, dalivery, bankTransfer, cod, currency, size } = pr;

select.addEventListener("input", (e) => {
  const option = e.target.value;
  if (option == "6-inches") updatePriceAndSizeValue(0);
  if (option == "7-inches") updatePriceAndSizeValue(1);
  if (option == "8-inches") updatePriceAndSizeValue(2);
});

function updatePriceAndSizeValue(num) {
  price = 96.98 + num * 2;
  size = 6 + num;
  updatePriceAndSize();
}
...
function updatePriceAndSize() {
  ...
  productPrice.forEach((p) => {
    p.innerHTML = `${price} <span class="section__currency">${currency}</span>`;
  });
}
